...
const SocraticQuestioningInputSchema = z.object({
  problem: z.string().describe('The problem or question the student needs help with.'),
  studentResponse: z.string().describe('The student\'s current response or solution attempt.'),
  stepByStepProgress: z.string().optional().describe('The step by step progress of the student.'),
});

export type SocraticQuestioningInput = z.infer<typeof SocraticQuestioningInputSchema>;

const SocraticQuestioningOutputSchema = z.object({
  question: z.string().describe('A probing question to guide the student towards understanding.'),
  ...
  updatedStepByStepProgress: z.string().optional().describe('The updated step by step progress of the student.'),
});

export type SocraticQuestioningOutput = z.infer<typeof SocraticQuestioningOutputSchema>;

export async function socraticQuestioning(input: SocraticQuestioningInput): Promise<SocraticQuestioningOutput> {
